import { Router } from 'express';
import { getClient } from '../config/db.js';

const router = Router();

/**
 * 13 Sequential Shop-Floor Production Stages
 */
const STAGES = [
  { id: 1, name: 'Yarn Purchase / Inward' },
  { id: 2, name: 'Winding & Doubling' },
  { id: 3, name: 'Warping & Creeling' },
  { id: 4, name: 'Sizing & Beam Preparation' },
  { id: 5, name: 'Drawing-in & Denting' },
  { id: 6, name: 'Loom Gaiting & Knotting' },
  { id: 7, name: 'Weaving / Loom Shed' },
  { id: 8, name: 'Doffing & Roll Numbering' },
  { id: 9, name: 'Grey Fabric Inspection (ASTM 4-Point)' },
  { id: 10, name: 'Mending & Cropping' },
  { id: 11, name: 'Folding, Rolling & Lapping' },
  { id: 12, name: 'Packaging, Baling & Weighting' },
  { id: 13, name: 'Finished Warehouse / Dispatch' }
];

/**
 * POST /api/production/handover
 * Executes stage handover with transactional audit logging and state transition
 */
router.post('/handover', async (req, res, next) => {
  const client = await getClient();
  
  try {
    const {
      batchId,
      fromStage,
      toStage,
      operatorId,
      rating,
      defectPayload = {},
      metersTransferred = 0,
      remarks = ''
    } = req.body;

    // 1. Validation Checks
    if (!batchId) {
      return res.status(400).json({ error: 'batchId is required.' });
    }

    const numericFromStage = parseInt(fromStage, 10);
    const numericToStage = parseInt(toStage, 10);
    const numericRating = parseInt(rating, 10);

    if (isNaN(numericFromStage) || numericFromStage < 1 || numericFromStage > 13) {
      return res.status(400).json({ error: 'Invalid fromStage (must be an integer between 1 and 13).' });
    }

    if (isNaN(numericToStage) || numericToStage < 1 || numericToStage > 13) {
      return res.status(400).json({ error: 'Invalid toStage (must be an integer between 1 and 13).' });
    }

    if (numericToStage <= numericFromStage) {
      return res.status(400).json({ error: 'toStage must be sequentially greater than fromStage.' });
    }

    if (isNaN(numericRating) || numericRating < 1 || numericRating > 5) {
      return res.status(400).json({ error: 'Quality rating is mandatory and must be an integer between 1 and 5 stars.' });
    }

    const parsedMeters = parseFloat(metersTransferred) || 0;
    if (parsedMeters < 0) {
      return res.status(400).json({ error: 'metersTransferred cannot be negative.' });
    }

    // 2. BEGIN EXPLICIT DATABASE TRANSACTION
    await client.query('BEGIN');

    // Verify batch exists and is currently active
    const batchCheckSql = `
      SELECT id, batch_number, current_stage_id, status 
      FROM production_batches 
      WHERE id = $1 
      FOR UPDATE
    `;
    const batchResult = await client.query(batchCheckSql, [batchId]);

    if (batchResult.rows.length === 0) {
      await client.query('ROLLBACK');
      return res.status(404).json({ error: `Production batch ${batchId} not found.` });
    }

    const batch = batchResult.rows[0];

    if (batch.status === 'COMPLETED' || batch.status === 'CANCELLED') {
      await client.query('ROLLBACK');
      return res.status(400).json({ error: `Cannot transfer batch with status: ${batch.status}.` });
    }

    const originStageName = STAGES.find(s => s.id === numericFromStage)?.name || `Stage ${numericFromStage}`;
    const targetStageName = STAGES.find(s => s.id === numericToStage)?.name || `Stage ${numericToStage}`;

    // 3. Write to stage_quality_audits (partitioned ledger)
    const auditInsertSql = `
      INSERT INTO stage_quality_audits (
        recorded_at,
        stage_id,
        origin_stage,
        target_stage,
        rating,
        inspector_id,
        defect_payload,
        remarks
      )
      VALUES (
        NOW(),
        $1,
        $2,
        $3,
        $4,
        $5,
        $6,
        $7
      )
      RETURNING audit_id, recorded_at, rating, defect_payload
    `;

    const auditRes = await client.query(auditInsertSql, [
      numericFromStage,
      originStageName,
      targetStageName,
      numericRating,
      operatorId || null,
      JSON.stringify(defectPayload),
      remarks
    ]);

    const auditId = auditRes.rows[0].audit_id;

    // 4. Write to immutable production_stage_transitions
    const transitionInsertSql = `
      INSERT INTO production_stage_transitions (
        from_stage,
        to_stage,
        operator_id,
        batch_id,
        audit_id,
        meters_transferred,
        transitioned_at
      )
      VALUES (
        $1,
        $2,
        $3,
        $4,
        $5,
        $6,
        NOW()
      )
      RETURNING transition_id, transitioned_at
    `;

    const transitionRes = await client.query(transitionInsertSql, [
      numericFromStage,
      numericToStage,
      operatorId || '00000000-0000-0000-0000-000000000000', // Default fallback UUID
      batchId,
      auditId,
      parsedMeters
    ]);

    // 5. Update production_batches stage and meterage
    const newStatus = numericToStage === 13 ? 'COMPLETED' : (numericRating <= 2 ? 'QUARANTINED' : 'ACTIVE');
    const updateBatchSql = `
      UPDATE production_batches
      SET 
        current_stage_id = $1,
        produced_meters = produced_meters + $2,
        status = $3,
        updated_at = NOW()
      WHERE id = $4
      RETURNING id, batch_number, current_stage_id, produced_meters, status
    `;

    const updatedBatchRes = await client.query(updateBatchSql, [
      numericToStage,
      parsedMeters,
      newStatus,
      batchId
    ]);

    // 6. COMMIT TRANSACTION
    await client.query('COMMIT');

    return res.status(200).json({
      success: true,
      message: `Batch ${batch.batch_number} successfully transitioned from ${originStageName} to ${targetStageName}`,
      data: {
        transition: transitionRes.rows[0],
        audit: auditRes.rows[0],
        batch: updatedBatchRes.rows[0]
      }
    });

  } catch (error) {
    // Ensure transaction is cleanly rolled back on any error
    try {
      await client.query('ROLLBACK');
    } catch (rollbackErr) {
      console.error('Error during rollback:', rollbackErr.message);
    }
    console.error('Production Handover Error:', error);
    return res.status(500).json({
      success: false,
      error: 'Transaction failed and was safely rolled back.',
      detail: error.message
    });
  } finally {
    // Release client connection back to pool
    client.release();
  }
});

export default router;
